...
当前部署侧支持 Go2 阶段 5 的 `45*6=270` 维历史观测 TorchScript 策略，单帧 45 维观测顺序和 `quadruped_train` 保持一致。训练侧 critic 的 48 维 privileged obs 只用于训练 value function，不进入本地 TorchScript 策略输入。当前普通 PPO actor 和后续 `estimator + actor` HIM 导出模型都必须保持外部输入为 270 维历史观测：
