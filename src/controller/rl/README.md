...
这个策略必须是当前 `quadruped_train` 阶段 7 导出的 270 维 Go2 历史观测 TorchScript 策略。训练侧 critic 的 238 维 privileged obs 只用于 value function 和 estimator，不会进入部署模型输入。阶段 7 的 JIT 内部包含 `estimator + actor`，但外部输入仍然必须保持 270 维历史观测；旧版 48 维 critic 配置、48 维 actor 输入或 45 维 actor 输入的策略和当前部署代码不兼容，需要重新训练并重新导出。
