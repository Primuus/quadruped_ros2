"""QLP serial protocol helpers for the RL remote input path."""
...
class QlpFrame:
    ...
    def __post_init__(self) -> None:
        ...
        if len(payload) > QLP_MAX_DATA_LEN:
            raise QlpProtocolError(f'data length {len(payload)} exceeds max {QLP_MAX_DATA_LEN}')
...
def pack_frame(frame: QlpFrame) -> bytes:
    ...
    header = struct.pack('<BBBBH', QLP_SOF, frame.addr, frame.src, frame.fc, frame.length)
...
def unpack_frame(raw: Union[bytes, bytearray, memoryview]) -> QlpFrame:
    ...
    if len(raw_bytes) != expected_size:
        raise QlpProtocolError(f'frame size {len(raw_bytes)} does not match LEN {data_len}')
...
def unpack_joystick_report(frame_or_raw: Union[QlpFrame, bytes, bytearray, memoryview]) -> JoystickReport:
    ...
    return JoystickReport(
        mode=mode,
        joy_lx=(joy_lx - 20.0) / 20.0,
        joy_ly=(joy_ly - 20.0) / 20.0,
        joy_rx=(joy_rx - 20.0) / 20.0,
        joy_ry=(joy_ry - 20.0) / 20.0,
        key=key,
    )
...
class QlpStreamParser:
    ...
    def parse_byte(self, byte: int) -> Optional[QlpFrame]:
        ...
        if self._state == self.STATE_DATA and len(self._buffer) >= QLP_HEADER_SIZE + self._data_len:
    ...
    def parse_bytes(self, data: Union[bytes, bytearray, memoryview, Iterable[int]]) -> list[QlpFrame]:
        frames = []
        for byte in data:
