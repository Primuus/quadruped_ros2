...
ConfigLoader::ConfigLoader(const std::string& config_file) 
    ...
    target_foot_pos_config_.setZero();
}
...
bool ConfigLoader::loadConfig() {
    ...
    success &= parseTargetFootPosParams(content);
...
void ConfigLoader::applyConfig(Mat3x4D& kp, Mat3x4D& kd, Vec4D& phase_t, Mat3x4D& target_foot_pos) {
    for (int i = 0; i < 4; i++) {
    ...
    for (int i = 0; i < 4; i++) {
        target_foot_pos.col(i) = target_foot_pos_config_.col(i);
    }
}
...
bool ConfigLoader::parseTargetFootPosParams(const std::string& content) {
    std::istringstream iss(content);
    std::string line;
    bool in_section = false;
    ...
    while (std::getline(iss, line)) {
        ...
        if (line.find("target_foot_pos_params:") != std::string::npos) {
            in_section = true;
            continue;
        }
        ...
        if (in_section && line.find("leg") == 0 && line.find(":") != std::string::npos && line[0] != '#') {
            ...
            if (values.size() == 3) {
                ...
                if (leg_index >= 0) {
                    target_foot_pos_config_.col(leg_index) << values[0], values[1], values[2];
                }
            }
        }
        
        if (in_section && line.find(":") != std::string::npos && 
            line.find("target_foot_pos_params:") == std::string::npos &&
            line.find("leg") != 0) {
            break;
        }
    }
